            ...
            child: ListView(
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                        child: _card('Total Classes', '$totalClasses',
                            Icons.school_outlined, Colors.blue)),
                    const SizedBox(width: 10),
                    Expanded(
                        child: _card('Total Students', '$totalStudents',
                ...
                Row(
                  children: [
                    Expanded(
                        child: _card('High Risk', '$highRisk',
                  ...
                  Card(
                    child: Padding(
                      ...
                      child: CompletionBarChart(
                        labels: classesList
                            .map((c) => c['class_name'] as String)
                            .toList(),
                        values: classesList
                            .map((c) =>
                                (c['avg_completion'] as num?)?.toDouble() ??
                                0.0)
                            .toList(),
                        caption:
                            'avg_completion per class — from GET /analytics/admin/overview',
                      ),
                    ),
                  ),
                  const SizedBox(height: 16),
                  const Text('Miss Rate by Class',
                      style:
                          TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 8),
                  Card(
                    child: Padding(
                      ...
                      child: CompletionBarChart(
                        ...
                        values: classesList
                            .map((c) =>
                                (c['avg_miss_rate'] as num?)?.toDouble() ?? 0.0)
                            .toList(),
                        caption:
                            'avg_miss_rate per class — from GET /analytics/admin/overview',
                      ),
                    ),
                  ),
                  const SizedBox(height: 16),
                  const Text('Drill Down by Class',
                      style:
                          TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                  const SizedBox(height: 8),
                  ...classesList.map((c) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        onTap: classId.isNotEmpty
                            ? () => context.push('/admin/analytics/$classId')
                            : null,
                      ),
                    );
                  }),
