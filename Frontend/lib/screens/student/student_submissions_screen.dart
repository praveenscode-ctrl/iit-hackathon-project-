...
class StudentSubmissionsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<StudentSubmissionsScreen> createState() => _StudentSubmissionsScreenState();
}

class _StudentSubmissionsScreenState extends ConsumerState<StudentSubmissionsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: mySubs.when(
        ...
        error: (e, _) => AppErrorWidget(message: e.toString(), onRetry: () => ref.invalidate(mySubmissionsProvider)),
        data: (list) {
          if (list.isEmpty) {
            return const Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
              Icon(Icons.history, size: 56, color: Color(0xFFD1D5DB)),
              SizedBox(height: 12),
              Text('No submissions found', style: TextStyle(fontSize: 15, color: Color(0xFF9CA3AF))),
            ]));
          }
          return RefreshIndicator(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (_, i) {
                final sub = list[i];
                final subId = sub.id;
                final title = sub.assignmentId; // Just assignment id for now since SubmissionModel doesn't have populated assignment title
                final type = sub.submissionType;
                final text = sub.textAnswer;
                final fileUrl = sub.fileUrl;
                final time = DateFormat('d MMM yyyy, h:mm a').format(sub.submittedAt.toLocal());
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('Submitted on: $time', style: const TextStyle(fontSize: 12, color: Color(0xFF6B7280))),
                        ...
                        if (fileUrl != null && fileUrl.isNotEmpty) ...[
                          ...
                          OutlinedButton.icon(
                            ...
                            label: Text(_downloading.contains(subId) ? 'Downloading...' : 'Download My File', style: const TextStyle(fontSize: 13)),
                          ),
                        ],
                      ],
                    ),
