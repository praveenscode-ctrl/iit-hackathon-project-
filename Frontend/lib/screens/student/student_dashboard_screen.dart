...
class _StudentDashboardScreenState
  ...
  void initState() {
    ...
    // Poll every 60 seconds for custom reminders / notifications
    _pollTimer = Timer.periodic(const Duration(seconds: 60), (timer) async {
      if (!mounted) return;
      final notifier = ref.read(notificationsProvider.notifier);
      final prevUnread = notifier.unreadCount;
      await notifier.load(silent: true);
      if (!mounted) return;
      final currentUnread = notifier.unreadCount;

      if (currentUnread > prevUnread) {
        final stateVal = ref.read(notificationsProvider).valueOrNull;
        if (stateVal != null && stateVal.isNotEmpty) {
          final latestNotif = stateVal.first;
          if (!latestNotif.isRead && mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                ...
                action: SnackBarAction(
                  ...
                  onPressed: () {
                    notifier.markRead(latestNotif.id);
                  },
                ),
              ),
            );
  ...
  Widget build(BuildContext context) {
    ...
    final assignments = user.classId != null
        ? ref.watch(assignmentsProvider(user.classId!))
        : null;
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          ...
          if (user.classId != null)
            ref.invalidate(assignmentsProvider(user.classId!));
          ...
          children: [
                        ...
                        children: [
                          Expanded(
                              child: _stat(
                                  ...
                                  Colors.green)),
                          Expanded(
                              child: _stat('Streak', '${a.currentStreak}🔥',
                                  Colors.purple)),
                          Expanded(
                              child: _stat(
                                  'Late', '${a.totalLate}', Colors.orange)),
                        ],
                      ),
                      const SizedBox(height: 16),
                      OutlinedButton(
            ...
            Row(
              ...
              children: [
                ...
                TextButton(
                    onPressed: () => context.push('/student/submissions'),
                    child: const Text('Past Submissions')),
              ],
            ),
            ...
            // Pending assignments
            if (user.classId == null)
              const Text('You are not assigned to a class yet.',
              ...
              assignments.when(
                ...
                data: (list) {
                  final pending = list
                      .where((a) =>
                          a.status == 'PUBLISHED' &&
                          (a.studentSubmission == null ||
                              !a.studentSubmission!.submitted))
                      .toList();
                  if (pending.isEmpty)
                    return const Text('Hooray! No pending assignments.',
                        style: TextStyle(color: Color(0xFF9CA3AF)));
                  return Column(
                    children: pending
                        .map((a) => AssignmentCard(
                              assignment: a,
                              onTap: () =>
                                  context.push('/student/assignments/${a.id}'),
                            ))
                        .toList(),
                  );
                },
