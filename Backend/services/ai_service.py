...
"""
    ...
    class_scoped_intents = {"who_missed_assignment", "student_completion_rate", "student_profile", "class_summary", "risk_students", "class_roster", "pending_approvals", "mentor_list", "assignment_list", "assignment_status", "assignment_analytics", "bottleneck_assignments", "late_submitters", "submission_history", "streak_leaders"}
    if intent in class_scoped_intents and not class_id:
        result = no_data_response("Please select a class before asking this question.")
    ...
    def get_assignment_by_ref(a_ref, t_ref):
        ...
        if a_ref:
            query = query.filter(Assignment.title.ilike(f"%{a_ref}%"))
        if t_ref:
    ...
    if intent == "who_missed_assignment":
        ...
        if not assignment_ref and not temporal_ref:
            result = no_data_response("Which assignment? Try: 'Who missed Assignment 1?' or 'Who missed yesterday's assignment?'")
        else:
            ...
            if not assignment:
                ...
                non_submitters = db.execute(text("""
                    ...
                    WHERE s.id IS NULL
                """), {"class_id": class_id, "assignment_id": str(assignment.id)}).fetchall()
        ...
        if not student_name:
            ...
            if student:
                subs = db.query(Submission, Assignment).join(Assignment).filter(
